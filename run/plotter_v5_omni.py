...
def plotter(): 
    ...
    # combined_bkg_bdt = combined_bkg[combined_bkg['omni_probs'] > 0.013]
    combined_bkg_bdt = combined_bkg[combined_bkg['omni_probs'] > 0.99877]
    combined_signal_bdt = combined_signal[combined_signal['omni_probs'] > 0.99877]
    ...
    # combined_bkg_bdt_mva = combined_bkg[combined_bkg['bdt_score_new'] > 0.045]
    combined_bkg_bdt_mva = combined_bkg[combined_bkg['bdt_score_new'] > 0.97]
    combined_signal_bdt_mva = combined_signal[combined_signal['bdt_score_new'] > 0.97]
    ...
    for i in range(4):
        fpr, tpr, thresholds = calc_roc(combined['signal'][channel[i]]['bdt_score_new'], combined['bkg'][channel[i]]['bdt_score_new'], combined['signal'][channel[i]]['event_weight'], combined['bkg'][channel[i]]['event_weight'])
        # fpr, tpr = calc_roc(combined['signal'][channel[i]]['bdt_score_new'], combined['bkg'][channel[i]]['bdt_score_new'])
        # print(tpr[(thresholds > 0.9) & (thresholds < 1) & (tpr > 0.7) & (tpr < 0.71)])
        # print("BBBBBBTTTTTT: ", thresholds[(thresholds > 0.9) & (thresholds < 1) & (tpr > 0.7) & (tpr < 0.71)])
        roc_auc = auc(fpr, tpr)
        plt.plot(tpr, 1/fpr, color=color[i], linewidth=1, alpha=0.7, label=f'{channel[i]} R22 BDT')
    ...
    for i in range(4):
        ...
        # fpr, tpr = calc_roc(combined['signal'][channel[i]]['omni_probs'], combined['bkg'][channel[i]]['omni_probs'])
        roc_auc = auc(fpr, tpr)
        # print(tpr[(thresholds > 0.9) & (thresholds < 1) & (tpr > 0.7) & (tpr < 0.71)])
        # print("TTTTTT: ", thresholds[(thresholds > 0.9) & (thresholds < 1) & (tpr > 0.7) & (tpr < 0.71)])
